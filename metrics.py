...
import numpy as np
# from medpy.metric import binary as medpy_metric
# from hausdorff import hausdorff_distance
import torch
from evaluation import *


def HD(batch_predictions, batch_labels):
    
    # print(batch_predictions.shape)
    # print(batch_labels.shape)

    # 将预测图和标签转换为二值化的形式
    binary_predictions = np.argmax(batch_predictions, axis=1)
    binary_labels = batch_labels
    # count = 0
    # a = torch.tensor(binary_predictions[0])
    # print(a)
    ...
    # raise ValueError
    WT_pred = np.where(binary_predictions[0] > 0, 1, 0)
    WT_label = np.where(binary_labels[0] > 0, 1, 0)
    WT_se_ = get_sensitivity(torch.tensor(WT_pred), torch.tensor(WT_label))
    WT_se_1 = get_sensitivity_BraTS(WT_pred, WT_label)

    print("WT_se: {}".format(WT_se_))
    print("WT_se_1: {}".format(WT_se_1))
    raise ValueError
    ...
    for i in range(0, batch_labels.shape[0]):
        ...
        WT_pred = np.where(binary_predictions[i] > 0, 1, 0)
        WT_label = np.where(binary_labels[i] > 0, 1, 0)
        WT_hd_ = hausdorff_distance(WT_pred, WT_label, distance='euclidean')
        WT_se_ = get_sensitivity(torch.tensor(WT_pred), torch.tensor(WT_label))
        WT_sp_ = get_specificity(torch.tensor(WT_pred), torch.tensor(WT_label))
        WT_dc_ = get_DC(torch.tensor(WT_pred), torch.tensor(WT_label))
        WT_hd.append(WT_hd_)
        WT_se.append(WT_se_)
        WT_sp.append(WT_sp_)
        WT_dc.append(WT_dc_)

        print(WT_hd, WT_se, WT_sp, WT_dc)
        raise ValueError

        #TC(tumor core): label 2+3 
        # TC_pred = np.where(binary_predictions[i] > 1, 1, 0)
        # TC_label = np.where(binary_labels[i] > 1, 1, 0)
        # TC_dis = hausdorff_distance(TC_pred, TC_label, distance='euclidean')
        # TC.append(TC_dis)

        # #ET(Enhancing tumor): label 3
        # ET_pred = np.where(binary_predictions[i] > 2, 1, 0)
        # ET_label = np.where(binary_labels[i] > 2, 1, 0)
        # ET_dis = hausdorff_distance(ET_pred, ET_label, distance='euclidean')
        # ET.append(ET_dis)
    ...
    # print("WT, TC, ET:", WT_mean, TC_mean, ET_mean)
    # return WT, TC, ET


class Dice_Cal(object):
    def __init__(self, ):
        self.reset()
        self.total_dice = 0
        self.number = 0
    ...
    def get(self):
        ...
        return self.total_dice/self.number
