...
def model_state_dict(model, args):
    return model.module.state_dict() if args.parallel == "DDP" else model.state_dict()


def save_model_state(model, args, filename):
    if not is_main_process():
        return
    torch.save(
        {"net": model_state_dict(model, args)},
        os.path.join(args.model_result_dir, filename),
    )
...
class MultiCropWrapper_v5(nn.Module):
    """
    Perform forward pass separately on each resolution input.
    The inputs corresponding to a single resolution are clubbed and single
    forward is run on the same resolution inputs. Hence we do several
    ...
    def __init__(self, backbone, heads):
        super(MultiCropWrapper_v5, self).__init__()
        backbone.fc, backbone.head = nn.Identity(), nn.Identity()
        self.backbone = backbone
        self.head = heads[3]
        self.head_1 = heads[0] # the projectors of the sub-encoders
        self.head_2 = heads[1]
        self.head_3 = heads[2]
        backbone.head_1 = nn.Identity()
        backbone.head_2 = nn.Identity()
        backbone.head_3 = nn.Identity()
    ...
    def forward(self, x):
        ...
        start_idx = 0
        for end_idx in idx_crops:
            _out_1, _out_2, _out_3, _out_4 = self.backbone(torch.cat(x[start_idx: end_idx]))
            ...
            if start_idx == 0:
                output_1, output_2, output_3, output_4 = _out_1, _out_2, _out_3, _out_4
            else:
                output_1 = torch.cat((output_1, _out_1))
                output_2 = torch.cat((output_2, _out_2))
                output_3 = torch.cat((output_3, _out_3))
                output_4 = torch.cat((output_4, _out_4))
            start_idx = end_idx
        # Run the head forward on the concatenated features.
        return [self.head_1(output_1), self.head_2(output_2), self.head_3(output_3),self.head(output_4)]

class MultiCropWrapper_v5_tea(nn.Module):
    """
    Perform forward pass separately on each resolution input.
    The inputs corresponding to a single resolution are clubbed and single
    forward is run on the same resolution inputs. Hence we do several
    ...
    """
    def __init__(self, backbone, heads):
        super(MultiCropWrapper_v5_tea, self).__init__()
        backbone.fc, backbone.head = nn.Identity(), nn.Identity()
        self.backbone = backbone
        self.head = heads[-1]
    ...
    def forward(self, x):
        ...
        start_idx = 0
        for end_idx in idx_crops:
            _out_4 = self.backbone(torch.cat(x[start_idx: end_idx]))
            if start_idx == 0:
                output_4 = _out_4
            else:
                output_4 = torch.cat((output_4, _out_4))
...
def get_split_KiTS(data_dir, fold, cross_vali_num, seed=12345):

    cases = os.listdir(data_dir)
    cases.sort()
    all_keys = np.arange(0, 156)
...
def get_split_AROI(data_dir, fold, cross_vali_num, seed=12345):
    
    cases = os.listdir(data_dir)
    cases.sort()
    all_keys = np.arange(1, 25)
...
def get_split_CANDI(data_dir, fold, cross_vali_num, seed=12345):
    
    cases = os.listdir(data_dir)
    cases.sort()
    all_keys = np.arange(1, 104)
...
def get_split_MSD(data_dir, fold, cross_vali_num, seed=12345):
    
    cases = os.listdir(data_dir)
    cases.sort()
    # print("cases: {}".format((cases)))
    # raise ValueError
    all_keys = np.arange(0, len(cases))
...
def get_split_CHAOS(data_dir, fold, cross_vali_num, seed=12345):
...
def get_split(data_dir, fold, cross_vali_num, seed=12345):
...
def get_split_RETOUCH(data_dir, fold, cross_vali_num, seed=12345):
    
    cases = os.listdir(data_dir)
    cases.sort()
    all_keys = np.arange(1, 71)
...
def get_split_BraTS(data_dir, fold, cross_vali_num, seed=12345):
    
    cases = os.listdir(data_dir)
    cases.sort()
    data_length = len(cases)
    print("data_length: {}".format(data_length))
    all_keys = np.arange(0, data_length)
    i = 0
    for case in cases:
...
def get_split_ISIC(data_dir, fold, cross_vali_num, seed=12345):
