...
class CHD(Dataset):
    ...
    def __getitem__(self, index):
        if self.do_contrast:
            ...
            #visualization---------------------------------
            #original image
            # image_norm = Normalization_result(image)
            # from PIL import Image
            # image_result = Image.fromarray((image_norm*255).astype(np.uint8))
            # model_path = os.path.join("visualization", "pretrain")
            # maybe_mkdir_p(model_path)
            # image_result.save(os.path.join(model_path, "original_image.png"))
            ...
            # from PIL import Image
            # image_result = Image.fromarray((image_norm*255).astype(np.uint8))
            # model_path = os.path.join("visualization", "pretrain", "{}".format(self.ssl_method))
            # maybe_mkdir_p(model_path)
            # image_result.save(os.path.join(model_path, "original_image.png"))
            
            # image_norm = Normalization_result(label)
            # from PIL import Image
            # image_result = Image.fromarray((image_norm*255).astype(np.uint8))
            # model_path = os.path.join("visualization", "pretrain", "{}".format(self.ssl_method))
            # maybe_mkdir_p(model_path)
            # image_result.save(os.path.join(model_path, "label.png"))
